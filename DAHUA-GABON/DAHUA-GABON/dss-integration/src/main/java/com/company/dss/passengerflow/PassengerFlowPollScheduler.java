...
/**
 * Poll périodique de l'historique People Counting (source réelle de l'UI / Excel DSS).
 * Complète le MQ : le comptage live client n'est pas forcément poussé en alarme MQ.
 */
@Slf4j
@Component
    ...
    public void pollAndLog() {
        ...
        try {
            ...
            if (snapshot.equals(lastSnapshot.get())) {
                log.info(">>> [FLOW] Poll OK — date={}, créneaux actifs={}, inchangé", date, active.size());
                return;
            }
            lastSnapshot.set(snapshot);

            log.info("============================================================");
            log.info(">>> [FLOW] Données People Counting (HTTP) — date={} — {} créneau(x) actif(s)",
                    date, active.size());
